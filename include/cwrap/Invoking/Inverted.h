...
/*
    An invertible functor family is a template taking
    one type argument where each specialization is a
    functor with an inverse operation, establishing a
    translation between two types, W and U:
    ...
    The parameter W is always a decayed type; a single
    specialization handles all references to W.

    This abstraction is used for both type wrappers and
    resource holders.  In the terminology of wrappers,
    W is the wrapped type, and U is the unwrapped type;
    in the terminology of resources, W is the seized
    resource type, and U us the released type.
    
    A central assumption in the design is that each
    wrapped type maps to exactly one unwrapped type,
    but
    ...
    If either operation returns a reference, it is
    assumed that modification of the referred-to-object
    corresponds to modification of the parameter.  That
    is, (ignoring const) when operator() returns a
    reference,
    ...
    When defining InvertibleFunctorFamilies, it is
    convenient that they are class templates: they can
    be defined piecemeal, putting full or partial
    spacializations with the types to which they apply.
    The Family InvertibleIdentityFunctor is a good
    starting point: it defines a do-nothing operation
    for every type.
    
    Sometimes it's handy to have a single class
    representing an entire family.  The Inverted
    template provides that: its operator() will unwrap
    or release any parameter while its Inverse<W>
    member wraps back to W.
*/
