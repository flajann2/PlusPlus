...
/*
    cwrap::Invoke calls a function, performing the
    steps necessary to adjust to its calling
    convention.  Because there are so many steps to the
    adjustment, it's perhaps the only function in cwrap
    that is really complicated.  These are the steps,
    grouped by the function that handles them:
    
    InvokeWithGroups (entering)
        Collect references to the items to be passed to
    ...
    For a full explanation of parameter and result
    groups, see InvokeWithGroups.h.  Some basic
    parameter groups are created by these functions
    from In.h, InOut.h, Out.h, and NotPassed.h:
