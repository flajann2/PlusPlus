...
/***
    InvokeWithGroups calls a function, rearranging its
    parameters, errors, and results according to
    parameter and result groups:
        
        The passed parts of the parameter groups are
        concatenated into a parameter list for the
        invoked function.  The function is invoked.
        
        If any group fails, the thrown parts of all
        groups are concatenated into a parameter list
        and passed to the failure functor.
        
        Otherwise, the returned parts of all groups are
        concatenated into a result tuple.
        ...
        When concatenating parts, parts from the result
        group come first, them parts from the parameter
        groups in order.
    ...
    A result group is a similar class that handles the
    function result, where r has type
    GroupType::ResultType.  If ResultType is void, no
    parameter is passed where r appears.
    ...
    Some basic parameter groups are provided in In.h,
    Out.h, and InOut.h.  A basic result group is
    provided by Result.h.
***/
